...
module("EmberLeaflet.MarkerCollectionLayer with controller", {
  setup: function() {
    content = Ember.A([
...
test("content is bound", function() {
  strictEqual(controller.get('content'), content,
    "controller content should be original array");
  strictEqual(collection.get('content'), controller,
    "collection should refer to controller");
  strictEqual(collection.get('content.content'), content,
